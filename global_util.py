...
class Global_MDP:
    ...
    def global_value_iteration(self):
        ...
        for s_idx, s_tuple in self.idx_to_state.items():
            ...
            self.V[s_idx, self.T] = X_prod * Y_prod
...
# ---------sum of prod and v in range sp in s_joint reachable set----------------
 ...
 # ---------sum of prod and v in range sp in s_joint reachable set----------------
            ...
            # Stop computation here and start after.
                ...
                # elif -> if s velow V threshold is for some rason in V get rid of it
                
                
                # Break these down from getting actions to getting Vs: for a in a_idx to v_sum=0.0, and for sp_idx to end
                # For Sarah's: 113-132 val policy: 71-104
                # If still faster, try to explain why. come up with story in simulation section. How many iterations are we doing? increase players as well
                # Try to answer does it reduce computation time and emm as state and players increase?
                # Fix statesd and players
                
                # After finding the best action we assign it to the value matrix V
                # read what you have in code and how does the local policy iterations scale with states, players, T
    ...
    def get_potential(self):
        ...
        self.potential_value = self.V[self.start_idx, 0]

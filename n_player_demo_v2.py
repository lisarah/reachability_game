...
MCs = 10 # monte carlo trials
action_entropy = 0.95
# ----- defining MDP -----#

Columns_list = [6,7,8,9,10,6,7,8,9,10]
Rows = 5
...
for tr in range(MCs):
    ...
    Columns = Columns_list[tr]
    state_computation_times = []
    State_data = []
    ...
    for ind in range(BR_iter):
        ...
        end_t = time.time()
        value_formation_time = np.round(end_t - begin_t, 2)
        print(f' value formation time is {value_formation_time} seconds')
        potential_eval_time.append(end_t - begin_t)
        ...
        end_t = time.time()
        value_iteration_time = np.round(end_t - begin_t, 2)
        print(f' value iteration time is {value_iteration_time}  seconds')
        ...
        state_computation_time = value_formation_time + value_iteration_time
        state_computation_times.append(state_computation_time)

        State_data.append(S)

    trial_potentials.append(potential)
    trial_no_collisions.append(no_collisions)
    trial_computation_times.append(sum(state_computation_times)/len(state_computation_times))
    ...
    print(f' length of data {len(potential)*2}')
    print(f' length of new data {len(state_computation_times*2)}')
    print(f' length of state data is {len( State_data*len(potential)*2)}')
    trial_data = pd.concat([trial_data, pd.DataFrame({
        ...
        'NumStates': [S]*len(potential)*2,
        'ComputationTime': state_computation_times*2})],
        ignore_index=True)
    
sns.set_style("darkgrid")
sns.relplot(kind="line",
    height = 4, aspect = 0.9,
    x="NumStates", y='ComputationTime', # style='type',   errorbar=("se", 5), 
    data=trial_data,
    # palette=sns.color_palette(),
);plt.show(block=True); plt.yscale('log'); # plt.xscale('log');  #  
